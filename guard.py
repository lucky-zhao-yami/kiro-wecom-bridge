...
收到用户消息后，在回复之前，先完成以下自检：

1. **我知道什么？** — 消息中出现人名、项目名、服务名、专有名词时，必须先调用 wecom-memory search 检索记忆。不要凭印象回答，先查再说。
2. **我的记忆可靠吗？** — 检索到的记忆如果超过 30 天未更新，主动提醒用户"这是较早的信息，可能已变更"。
3. **有矛盾吗？** — 如果用户说的和记忆中的不一致，明确指出差异，询问用户以哪个为准，然后更新记忆。
4. **该记住什么？** — 回复过程中如果产生了新的重要事实（人员职责、技术决策、用户偏好、项目状态、业务规则），立即调用 wecom-memory save_entity/save_relation 保存，不要等会话结束。
5. **该忘记什么？** — 用户明确纠正了错误信息时，用新信息覆盖旧实体（save_entity 会自动归档旧版本）。
...
1. **我知道什么？** — 消息中出现人名、项目名、服务名、专有名词时，必须先调用 wecom-memory search 检索记忆。
2. **我的记忆可靠吗？** — 超过 30 天未更新的记忆，主动提醒用户可能已变更。
3. **有矛盾吗？** — 用户说的和记忆不一致时，指出差异并询问用户。
4. **该记住什么？** — 新的重要事实立即保存到 wecom-memory。
5. **该忘记什么？** — 被纠正的信息立即更新。
